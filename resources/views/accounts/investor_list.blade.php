...
@section('content')
    <div class="row p-2">
        <div class="col-sm-10 col-lg-10 col-md-10">
            <p class="card-title-desc">Enlisted projects showing below. We have
                <code> total {{$projects->where('is_investor_project',1)->count()}} projects</code>.
            </p>
        </div>
        <div class="col-sm-2 col-lg-2 col-md-2">
            <div class="row">
                <a href="{{route('project.create',['is_investor'=> 1])}}" class="ml-auto btn btn-secondary px-4" title="Add material"><i class="fas fa-plus font-size-16"></i></a>
            </div>
        </div>
    </div>
    ...
    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
        ...
        @foreach($projects as $project)
            <tr>
                <td>{{$loop->index+1}}</td>
                <td><a href="{{route('project.investor.account.show',$project->id)}}">{{$project->name}}</a></td>
